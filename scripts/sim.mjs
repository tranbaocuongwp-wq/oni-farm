...
test("142. BẢN ĐỒ CAO GẤP ĐÔI: nối thêm xuống dưới, và save cũ không mất một ô nào", () => {
  /* Lời hứa của đợt này nằm gọn trong một câu: "nối thêm xuống dưới để save cũ
     không mất gì". Nối xuống DƯỚI chứ không phải chèn vào giữa hay dịch cả
     lưới — vì `mergeGrid` ánh xạ theo (x,y), nên mọi ô cũ giữ nguyên toạ độ và
     mọi thứ người chơi đã làm ở đó đi theo. Dịch lưới một hàng thôi là cả nông
     trại lệch một ô: luống cày, cây đang lớn, công trình đã xây.

     Kịch bản này canh đúng chỗ ấy, và canh bằng cách khó chối nhất: dựng một
     ván chơi đã cày cấy xây dựng trên lưới CŨ, rồi migrate sang content MỚI và
     đòi từng ô một phải khớp. */

  const s = mkStore().getState();
  ok(s.h > 60, `bản đồ phải cao hơn hẳn bản cũ (37 hàng), đang ${s.h}`);
  eq(s.w, 48, "bề NGANG không được đổi — đổi ngang là lệch toàn bộ chỉ số ô");

  /* --- Phần MỚI có đủ ba thứ đã hứa: sông, cầu, biển ------------------- */
  const dem = (f) => {
    let n = 0;
    for (let y = 37; y < s.h; y++) for (let x = 0; x < s.w; x++) if (f(tileAt(s, x, y))) n++;
    return n;
  };
  ...
  /* CÂY CẦU PHẢI CÓ NGHĨA — và đây là cách duy nhất đo được điều đó.
     ...
     Bản đầu tôi viết "phải có một hàng toàn nước cắt ngang". Phép kiểm ấy
     XANH cả khi con sông bị chọc thủng một lỗ lội qua được — vì nó bắt trúng
     mấy hàng BIỂN ở đáy bản đồ, không phải con sông. Một phép kiểm đo nhầm đối
     tượng thì xanh cũng vô nghĩa.
     ...
     tới được thì ở đâu đó có lối lội vòng, và cây cầu chỉ là trang trí. */
  {
    ...
    let boNam = null;
    for (let y = cauDau.y + 1; y < s.h && !boNam; y++) {
      const t = tileAt(s, cauDau.x, y);
      if (t && t.g !== "water") boNam = { x: cauDau.x, y };
    }
    ok(!!boNam, "phải tìm được bờ nam của sông");

    const tu = { x: Math.floor(s.player.x / TILE), y: Math.floor(s.player.y / TILE) };
    const dich = new Set([idx(s.w, boNam.x, boNam.y)]);
    ok(
      !!findPath(s, content, tu.x, tu.y, dich, { maxNodes: 20000 }),
      "CÓ cầu thì phải đi bộ sang được bờ nam",
    );
  ...
  /* --- ĐI TỚI ĐƯỢC: qua cầu, sang bờ nam ------------------------------- */
  {
    // ô cầu đầu tiên, và ô đất ngay dưới bờ nam của sông
    let cau = null;
    for (let y = 37; y < s.h && !cau; y++)
      for (let x = 0; x < s.w; x++) if (tileAt(s, x, y)?.prop === "pier") { cau = { x, y }; break; }
    ok(!!cau, "phải tìm được ô cầu");
    ok(!isSolid(s, content, cau.x, cau.y), "cầu phải ĐI QUA ĐƯỢC — cầu chặn đường thì không phải cầu");

    const dich = new Set([idx(s.w, cau.x, cau.y)]);
    const duong = findPath(s, content, Math.floor(s.player.x / TILE), Math.floor(s.player.y / TILE), dich, {
      maxNodes: 12000,
    });
    ok(!!duong && duong.length > 0, "từ chỗ đứng đầu ván phải có đường đi bộ tới cây cầu");
  }

  /* --- RỪNG NAM là khu chặt được thật, không phải tranh vẽ ------------- */
  {
    const kn = (content.tiles.zones ?? []).filter((z) => z.kind === "forest" && z.y > 37);
    ok(kn.length >= 1, "rừng nam phải được khai là một khu `forest`");
    const z = kn[0];
    let cay = null;
    for (let y = z.y; y < z.y + z.h && !cay; y++)
      for (let x = z.x; x < z.x + z.w; x++) if (tileAt(s, x, y)?.prop === "tree") { cay = { x, y }; break; }
    ok(!!cay, "rừng nam phải có cây");
    ok(inZone(s, content, "forest", cay.x, cay.y), `cây ở (${cay.x},${cay.y}) phải nằm trong khu rừng`);
  }

  /* --- LỜI HỨA CHÍNH: save trên lưới CŨ migrate sang lưới MỚI không mất gì */
  {
    const store = mkStore(777);
    // Làm một ván có dấu vết thật ở nhiều chỗ trên phần CŨ
    store.dispatch({ t: "DEBUG", op: "money", n: 9999 });
    store.dispatch({ t: "DEBUG", op: "tillMap" });
    store.dispatch({ t: "DEBUG", op: "plantMap" });
    store.dispatch({ t: "DEBUG", op: "waterMap" });
    for (let i = 0; i < 600; i++) store.dispatch({ t: "TICK", dt: 1 / 60 });

    const truoc = store0(store);
    const CU_H = 37;
    ok(truoc.h > CU_H, "ván này đã chạy trên lưới MỚI");

    /* Giả lập một save của bản CŨ: cắt lưới về đúng 37 hàng đầu, và khai
       h = 37. Đó chính là hình dạng file save của người chơi trước đợt này. */
    const cu = JSON.parse(JSON.stringify(truoc));
    cu.h = CU_H;
    cu.tiles = truoc.tiles.slice(0, truoc.w * CU_H);
    eq(cu.tiles.length, truoc.w * CU_H, "save giả lập phải đúng 37 hàng");

    const kq = migrateForContent(cu, content);
    ok(!!kq.state, "save cũ phải nạp được, không được từ chối");
    const sau = kq.state;
    eq(sau.h, truoc.h, "nạp xong phải cao bằng bản đồ mới");
    eq(sau.w, truoc.w, "bề ngang không đổi");

    /* TỪNG Ô của phần cũ phải khớp: cày, ẩm, cây, giai đoạn cây, công trình. */
    let lech = 0;
    let viDu = null;
    for (let y = 0; y < CU_H; y++)
      for (let x = 0; x < truoc.w; x++) {
        const a = cu.tiles[y * cu.w + x];
        const b = sau.tiles[y * sau.w + x];
        const nhu = (t) => (t ? `${t.tilled ? 1 : 0}${t.wet ? 1 : 0}|${t.crop?.id ?? "-"}|${t.crop?.stage ?? "-"}|${t.b ?? "-"}` : "?");
        if (nhu(a) !== nhu(b)) {
          lech++;
          if (!viDu) viDu = `(${x},${y}) cũ=${nhu(a)} mới=${nhu(b)}`;
        }
      }
    eq(lech, 0, `phần cũ phải khớp TỪNG Ô sau khi migrate; lệch ${lech} ô, ví dụ ${viDu}`);

    /* Và phần MỚI phải được dựng ra, không phải để trống. */
    let coDat = 0;
    for (let y = CU_H; y < sau.h; y++)
      for (let x = 0; x < sau.w; x++) if (sau.tiles[y * sau.w + x]) coDat++;
